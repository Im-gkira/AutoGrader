...
def predict(img, x1, y1, x2, y2, proba=False, acc_thresh=0.60):
    ...
    contours_tmp, hierarchy = cv2.findContours(
        temp_tmp, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def extract_box(img, show=True):
    ...
    (thresh, img_final_bin) = cv2.threshold(
        img_final_bin, 0, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)

    # Find and sort the contours
    if(cv2.__version__ == '3.3.1'):
        xyz, contours, hierarchy = cv2.findContours(
...
def extract_line(image, beta=0.7, alpha=0.002, show=True):
    ...
    H, W = img.shape[:2]
    uppers = np.array(
        [y for y in range(H-1) if hist[y] <= th and hist[y+1] > th])
    lowers = np.array(
        [y for y in range(H-1) if hist[y] > th and hist[y+1] <= th])

    diff_1 = np.array([j-i for i, j in zip(uppers, lowers)])
    diff_index_1 = np.array(
        [True if j > beta*(np.mean(diff_1)-np.std(diff_1)) else False for j in diff_1])
...
def text_segment(Y1, Y2, X1, X2, box_num, line_name, dict_clean,
    ...
    for c in contours:
        if(cv2.contourArea(c)**2 > ct_th):
            cnts.append(c)
    contours_sorted, bounding_boxes = sort_contours(
    ...
    while i in range(0, len(contours_sorted)):
        ...
        # Normalised location of char w.r.t box image
